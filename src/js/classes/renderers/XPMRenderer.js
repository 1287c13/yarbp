...
export class XPMRenderer extends YarbpBasicRenderer {
  static DEFAULTS = Object.freeze({
    ...
    // Таблица решений
    DECISION_TABLE_OFFSET_X: 10,
    DECISION_TABLE_OFFSET_Y: -35,
  ...
  static getColor(colorType = 'main') {
    ...
    switch (colorType) {
      ...
      case 'ide-code-val':
      default:
        return isDark ? XPMRenderer.DEFAULTS.COLOR_MAIN_DARK : XPMRenderer.DEFAULTS.COLOR_MAIN_LIGHT;
    }
  }
  ...
  static normalizeArrow(cfg) {
    return cfg || {
      show: false, style: 'solid', hasMarker: false, hasInMarker: false
    };
  }
  ...
  /**
   * Строит SVG-путь обходной дуги для «обратной» стрелки таблицы решений.
   * Точки: (x0,y0) → (x1,y0) → (x1,y1) → (x2,y1) → (x2,y2).
   * Углы скругляются квадратичными кривыми радиуса r, если сегменты достаточно длинные.
   */
  static _buildBackLoopPath(x0, y0, x1, y1, x2, y2, r) {
    const parts = [`M ${x0} ${y0}`];

    const horiz1 = Math.abs(x1 - x0);
    const vert1 = Math.abs(y0 - y1);
    const horiz2 = Math.abs(x1 - x2);
    const vert2 = Math.abs(y2 - y1);

    // Поворот 1: (x1, y0)
    if (r > 0 && horiz1 > r && vert1 > r) {
      parts.push(`L ${x1 - r} ${y0}`);
    ...
    // Поворот 2: (x1, y1)
    if (r > 0 && vert1 > r && horiz2 > r) {
      parts.push(`L ${x1} ${y1 + r}`);
    ...
    // Поворот 3: (x2, y1)
    if (r > 0 && horiz2 > r && vert2 > r) {
      parts.push(`L ${x2 + r} ${y1}`);
      parts.push(`Q ${x2} ${y1} ${x2} ${y1 + r}`);
    } else {
      parts.push(`L ${x2} ${y1}`);
    }
  ...
  /**
   * Сдвигает конец стрелки назад, чтобы маркер «всплыл» из целевой точки.
   * Направление — от точки назначения (x2, y2) к предыдущей точке кривой (x1, y1).
   *
   * @param {number} x1 — предпоследняя точка кривой
   * @param {number} y1
   * @param {number} x2 — конец кривой (центр целевой точки)
   * @param {number} y2
   * @param {number} trim — на сколько пикселей отступить от (x2, y2)
   * @returns {{x: number, y: number}}
   */
  static _trimArrowEnd(x1, y1, x2, y2, trim) {
    const dx = x2 - x1;
    const dy = y2 - y1;
    const len = Math.hypot(dx, dy);
    if (len < 1e-6) return {x: x2, y: y2};
    return {
      x: x2 - (dx / len) * trim, y: y2 - (dy / len) * trim
    };
  }
  ...
  getMarkersDefs() {
    ...
    ['Solid', 'Dashed', 'Dotted'].forEach(type => {
      ...
      markerOut.appendChild(XPMRenderer.createSvgElement('path', {
        d: 'M 0 0 L 10 5 L 0 10 z', fill: XPMRenderer.getColor()
      }));
      ...
      markerIn.appendChild(XPMRenderer.createSvgElement('path', {
        d: 'M 10 0 L 0 5 L 10 10 z', fill: XPMRenderer.getColor()
      }));
      ...
      point: {
        measure(config) {
          ...
          if (config.decisionTable) {
            ...
            const tableLeft = XPMRenderer.DEFAULTS.DOT_BASE_X + XPMRenderer.DEFAULTS.DECISION_TABLE_OFFSET_X;
            const tableRight = tableLeft + tableSize.totalWidth;
            const tableTop = XPMRenderer.DEFAULTS.DOT_BASE_Y + XPMRenderer.DEFAULTS.DECISION_TABLE_OFFSET_Y;
        ...
        buildStaticElements(config) {
          const {
            pointStyle = 'filled', title = '', listText = '', arrows = {}
          } = config;
        ...
        render(config, cellWidth, cellHeight) {
          ...
          if (bypassEnabled) {
            const styleForArc = (arrows['left']?.show && arrows['left'].style !== 'dotted') ? arrows['left'].style : (arrows['top']?.show && arrows['top'].style !== 'dotted') ? arrows['top'].style : 'solid';
      ...
      lines: {
        measure() {
          ...
          if (hLine?.show) {
            ...
            group.appendChild(XPMRenderer.drawSimpleLine(0, y, cellWidth, y, hLine.style || 'solid'));
          }
          if (vLine?.show) {
            const x = vLine.x !== undefined ? vLine.x : XPMRenderer.DEFAULTS.DOT_BASE_X;
            group.appendChild(XPMRenderer.drawSimpleLine(x, 0, x, cellHeight, vLine.style || 'solid'));
          }
      ...
      image: {
        measure(config) {
          ...
          const size = {
            ...
            minHeight: Math.max(XPMRenderer.DEFAULTS.IMAGE_HEIGHT, XPMRenderer.DEFAULTS.TITLE_FONT_SIZE * 2),
            minX: 0,
            minY: 0
          };
          XPMRenderer.setCachedSize(config, size);
        ...
        render(config, cellWidth, cellHeight) {
          ...
          const resolvedSrc = src && (src.startsWith('http://') || src.startsWith('https://') || src.startsWith('data:')) ? src : XPMRenderer._createPinSVGDataURI(XPMRenderer.getColor(), src);
          ...
          return {
            group, minWidth: size.minWidth, minHeight: size.minHeight
          };
        }
      },
      ...
      divider: {
        measure(config) {
          ...
          const size = {
            ...
            minHeight: lines.length * XPMRenderer.DEFAULTS.DIVIDER_LINE_HEIGHT + XPMRenderer.DEFAULTS.DIVIDER_OFFSET_Y,
            minX: 0,
            minY: 0
          };
          XPMRenderer.setCachedSize(config, size);
  ...
  _renderDecisionArrows(svg, tiles, offsetX, offsetY, colMaxWidth, rowMaxHeight) {
    const trim = XPMRenderer.DEFAULTS.DOT_RADIUS + XPMRenderer.DEFAULTS.DECISION_ARROW_END_CLEARANCE;
    ...
    tiles.forEach(tile => {
      ...
      const tableSize = XPMRenderer._measureDecisionTable(table);
      const tableX = dotX + XPMRenderer.DEFAULTS.DECISION_TABLE_OFFSET_X;
      const tableY = dotY + XPMRenderer.DEFAULTS.DECISION_TABLE_OFFSET_Y;

      const startXBase = tileOffsetX + tableX + tableSize.totalWidth;
      const rowsY0 = tileOffsetY + tableY + tableSize.headerHeight;
      const rowHeight = XPMRenderer.DEFAULTS.DECISION_TABLE_ROW_HEIGHT;

      table.rows.forEach((row, rowIndex) => {
        if (!row.targetGrid) return;

        const targetTile = tiles.find(t => t.grid.x === row.targetGrid.x && t.grid.y === row.targetGrid.y);
        ...
        const startY = rowsY0 + rowIndex * rowHeight + rowHeight / 2;

        const endX = offsetX[row.targetGrid.x] + XPMRenderer.DEFAULTS.DOT_BASE_X + targetInnerX;
        const endY = offsetY[row.targetGrid.y] + XPMRenderer.DEFAULTS.DOT_BASE_Y + targetInnerY;

        let d;

        if (endX < startX) {
          // Обратный случай: обходная дуга через верх
          const backOffsetX = XPMRenderer.DEFAULTS.DECISION_ARROW_BACK_OFFSET_X;
          ...
          const loopX = startX + backOffsetX;
          const proposedLoopY = endY - backMarginTop;
          const tableTopY = rowsY0;
          const loopY = Math.max(proposedLoopY, tableTopY - backMarginTop);

          // Последний сегмент дуги — вертикальный вниз, поэтому конец
          // сдвигаем строго вверх на trim.
          const trimmedEndY = endY - trim;

          d = XPMRenderer._buildBackLoopPath(startX, startY, loopX, loopY, endX, trimmedEndY, backRadius);
        } else {
          // Прямой случай: кубическая безье
