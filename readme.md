...
```abnf
; ============================================================
; ЯРБП v2.0 — ABNF спецификация
; ============================================================

документ       = *(пустая-строка / строка-элемента) [строка-элемента]

; --- Основная структура ---
строка-элемента = отступ элемент конец-строки
элемент         = директива / узел / атрибут-строкой / массив-строкой

; --- Директивы ---
директива      = "!" отступ-внутри тип-директивы значение-директивы
тип-директивы  = "как" / "as"
значение-директивы = 1*(ALPHA / DIGIT / "-" / "_")

; --- Узлы (node) ---
узел           = имя-узла *(WS атрибут) [комментарий] конец-строки
                 *(отступ дочерний-элемент)

имя-узла       = (ALPHA / "_") *(ALPHA / DIGIT / "_" / "-")

; --- Атрибуты ---
атрибут        = ключ-атрибута [":" тип-значения] значение-атрибута
атрибут-строкой = *WS атрибут *(WS атрибут) [комментарий] конец-строки

ключ-атрибута  = "." идентификатор-ключа / ":" идентификатор-ключа
идентификатор-ключа = (ALPHA / "_") *(ALPHA / DIGIT / "_" / "-")

; --- Типизация ---
тип-значения   = идентификатор-типа
идентификатор-типа = (ALPHA / "_") *(ALPHA / DIGIT / "_" / "-")

значение-атрибута = литерал

; --- Массивы ---
массив-строкой = *WS ".." [идентификатор-ключа] *(WS значение-массива) [комментарий] конец-строки
                 *(отступ строка-массива)
строка-массива = *WS значение-массива [комментарий] конец-строки
значение-массива = литерал

; --- Дочерние элементы ---
дочерний-элемент = строка-элемента / строка-массива

; --- Литералы ---
литерал        = число / строка / булево / идентификатор-без-кавычек

число          = ["-"] 1*DIGIT ["." 1*DIGIT]
строка         = одиночная-кавычка *(не-кавычка) одиночная-кавычка
               / двойная-кавычка *(не-двойная-кавычка) двойная-кавычка
одиночная-кавычка = %x27
двойная-кавычка   = %x22
не-кавычка        = %x00-26 / %x28-7F / %x80-10FFFF
не-двойная-кавычка = %x00-21 / %x23-7F / %x80-10FFFF

булево         = "yes" / "no" / "true" / "false" / "да" / "нет"

идентификатор-без-кавычек = (ALPHA / "_") *(ALPHA / DIGIT / "_" / "-" / ".")

; --- Комментарии ---
комментарий    = 1*пробел-внутри "--" *(SP / VCHAR / UTF8-символ)

; --- Отступы и пробелы ---
отступ         = *WS
отступ-внутри  = 1*WS
конец-строки   = *WS [комментарий] (CRLF / LF / CR)

пустая-строка  = *WS (CRLF / LF / CR)
...
DIGIT          = %x30-39
ALPHA          = %x41-5A / %x61-7A
ALPHA          =/ %x410-42F / %x430-44F / %x401 / %x451
UTF8-символ    = %x80-10FFFF
VCHAR          = %x21-7E
CRLF           = CR LF
CR             = %x0D
LF             = %x0A
